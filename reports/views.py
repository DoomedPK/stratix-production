...
# -----------------------------------------------------------------------------
# STRATIX DRONEDEPLOY AUTOMATION (WEBHOOK LISTENER)
# -----------------------------------------------------------------------------
@csrf_exempt
def dronedeploy_webhook(request):
    """
    Listens for DroneDeploy 'export.complete' events and automatically 
    attaches the 3D model link to the corresponding Stratix Site Report.
    """
    if request.method == 'POST':
        try:
            # Parse the incoming JSON from DroneDeploy
            data = json.loads(request.body)
            
            # Check if this is the event that contains the finished model/export
            if data.get('event') == 'export.complete':
                # DroneDeploy sends the project name, which should match our site_id
                site_id_val = data.get('data', {}).get('project_name') 
                # The public link to view the 3D model
                model_link = data.get('data', {}).get('download_path') 
                
                if site_id_val and model_link:
                    # Find the site in the database
                    site = Site.objects.filter(site_id=site_id_val).first()
                    
                    if site:
                        # Grab the active report and attach the 3D model link
                        report = site.reports.first()
                        if report:
                            report.drone_3d_model_link = model_link
                            report.save()
                            
                            # Ping the global dashboard so clients see it instantly
                            # We use the first superuser as the "system" trigger for the alert
                            system_user = User.objects.filter(is_superuser=True).first()
                            if system_user:
                                ActivityAlert.objects.create(
                                    message="DroneDeploy 3D Model generated and linked automatically.", 
                                    user=system_user, 
                                    site=site, 
                                    alert_type='UPLOAD'
                                )
            ...
            # Fail silently to the outside world, but log it
            print(f"DroneDeploy Webhook Error: {str(e)}")
